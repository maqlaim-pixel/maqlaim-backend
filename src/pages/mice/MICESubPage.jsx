...
import {
  MICE_MEETINGS, MICE_INCENTIVES, MICE_CONFERENCES, MICE_EXHIBITIONS, MICE_SUPPORT, DEFAULT_PAGE
} from '../../data/pageData'

const SECTION_MAP = {
  'meetings':     MICE_MEETINGS,
  'incentives':   MICE_INCENTIVES,
  'conferences':  MICE_CONFERENCES,
  'exhibitions':  MICE_EXHIBITIONS,
  'events':       MICE_EXHIBITIONS,
  'support':      MICE_SUPPORT,
}

const HERO_BG = 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1400'

export default function MICESubPage() {
  const location = useLocation()
  const pathParts = location.pathname.split('/').filter(Boolean)
  const section = pathParts[1] || 'meetings'
  const subSlug = pathParts[2] || 'default'

  const dataMap = SECTION_MAP[section] || MICE_MEETINGS
    ...
    <div className="min-h-screen">
      <section className="relative h-[60vh] min-h-[400px] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center text-white px-4 max-w-4xl mx-auto">
          <div className="flex items-center justify-center gap-2 mb-4 text-sm">
            <Link to="/mice" className="text-white/70 hover:text-white">MICE</Link>
      ...
      <section className="section-padding bg-white">
        <div className="container-wide">
          <h2 className="text-3xl font-display font-bold text-navy-900 mb-8 text-center">Our Services</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {pageData.highlights?.map((item, i) => (
              <div key={i} className="bg-gray-50 rounded-2xl p-6 hover:shadow-lg transition-shadow text-center group">
                <div className="w-16 h-16 bg-indigo-100 rounded-full flex items-center justify-center mx-auto mb-4"><Compass size={28} className="text-indigo-600" /></div>
                <h3 className="font-bold text-navy-900 text-lg mb-2">{item}</h3>
                <p className="text-navy-500 text-sm">Professional {item.toLowerCase()} services</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="container-wide">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Object.entries(dataMap).filter(([k]) => k !== 'default').map(([slug, data]) => (
                <Link key={slug} to={`/mice/${section}/${slug}`} className="bg-white rounded-2xl p-6 hover:shadow-lg transition-all group border border-gray-100 hover:border-indigo-200">
                  <span className="text-4xl block mb-3">{data.emoji}</span>
                  <h3 className="font-bold text-navy-900 text-lg mb-2 group-hover:text-indigo-600 transition-colors">{data.title}</h3>
                  <p className="text-navy-500 text-sm mb-4">{data.desc}</p>
                  <span className="text-indigo-600 font-semibold text-sm flex items-center gap-1">Explore <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" /></span>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div className="container-wide">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div>
              ...
              <p className="text-navy-500 mb-8">Contact our MICE experts for professional event planning solutions.</p>
              <div className="space-y-5">
                ...
                <div className="flex items-center gap-4"><div className="bg-indigo-100 p-3 rounded-xl"><Mail size={20} className="text-indigo-600" /></div><div><p className="text-sm text-navy-500">Email Us</p><p className="font-semibold text-navy-900">mice@travelvista.com</p></div></div>
              </div>
            </div>
            <div className="bg-gray-50 rounded-2xl shadow-lg p-8">
              <h3 className="text-xl font-bold text-navy-900 mb-6">Enquire Now</h3>
              <EnquiryForm destination={pageData.title} theme="indigo" />
            </div>
          </div>
        </div>
      </section>
