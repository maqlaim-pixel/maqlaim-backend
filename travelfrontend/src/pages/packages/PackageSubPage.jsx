...
import { PACKAGE_DEFAULTS, DEFAULT_PAGE } from '../../data/pageData'

const HERO_BG = 'https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=1400'

export default function PackageSubPage() {
  const location = useLocation()
  const pathParts = location.pathname.split('/').filter(Boolean)
  const category = pathParts[1] || 'default'
  const subSlug = pathParts[2] || 'default'

  const pageData = PACKAGE_DEFAULTS[category] || DEFAULT_PAGE
    ...
    <div className="min-h-screen">
      <section className="relative h-[60vh] min-h-[400px] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center text-white px-4 max-w-4xl mx-auto">
          <div className="flex items-center justify-center gap-2 mb-4 text-sm">
            <Link to="/packages" className="text-white/70 hover:text-white">Packages</Link>
      ...
      <section className="section-padding bg-white">
        <div className="container-wide">
          <h2 className="text-3xl font-display font-bold text-navy-900 mb-8 text-center">What We Offer</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {pageData.highlights?.map((item, i) => (
              <div key={i} className="bg-gray-50 rounded-2xl p-6 hover:shadow-lg transition-shadow text-center group">
                <div className="w-16 h-16 bg-sky-100 rounded-full flex items-center justify-center mx-auto mb-4"><Compass size={28} className="text-sky-600" /></div>
                <h3 className="font-bold text-navy-900 text-lg mb-2">{item}</h3>
                <p className="text-navy-500 text-sm">Explore {item.toLowerCase()} options</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section-padding bg-gray-50">
        <div className="container-wide">
          <h2 className="text-3xl font-display font-bold text-navy-900 mb-8 text-center">Packages Coming Soon</h2>
        ...
        <div className="container-wide">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-display font-bold text-navy-900 mt-2 mb-6">Find Your Perfect Package</h2>
              <p className="text-navy-500 mb-8">Contact our travel experts for the best {pageData.title.toLowerCase()} deals.</p>
