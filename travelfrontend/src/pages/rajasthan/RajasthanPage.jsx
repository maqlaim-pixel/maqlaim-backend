...
import { Castle, Crown, Mountain, Music, UtensilsCrossed, TreePalm } from 'lucide-react'
...
const WHY_RAJASTHAN = [
  { Icon: Castle, title: 'Majestic Forts', desc: 'Amber Fort, Mehrangarh & more' },
  { Icon: Crown, title: 'Royal Palaces', desc: 'Lake Palace, City Palace & havelis' },
  { Icon: Mountain, title: 'Thar Desert', desc: 'Camel safari, sand dunes & desert camps' },
  { Icon: Music, title: 'Vibrant Culture', desc: 'Folk music, dance, crafts & festivals' },
  { Icon: UtensilsCrossed, title: 'Royal Cuisine', desc: 'Dal Bati, Laal Maas & royal thali' },
  { Icon: TreePalm, title: 'Wildlife Safari', desc: 'Ranthambore, Sariska & Keoladeo' },
]
...
export default function RajasthanPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const rajDests = destRes.data.filter(d =>
        d.state?.toLowerCase() === 'rajasthan'
      )
      const rajPkgs = pkgRes.data.filter(p =>
        p.state?.toLowerCase() === 'rajasthan' || p.destination?.toLowerCase().includes('rajasthan')
      )
      setDestinations(rajDests)
      setPackages(rajPkgs)
  ...
  return (
    <div>
          ...
          <div key={i} className={`absolute inset-0 transition-opacity duration-1000 ${i === currentSlide ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8 md:p-16 text-white">
              <div className="container-wide">
                <p className="text-gold-400 text-sm font-medium mb-2">🏰 Forts • Palaces • Desert Adventures</p>
                ...
                <p className="text-gray-300 mt-3 max-w-xl text-sm md:text-base">From the majestic forts of Jaipur to the golden sands of Jaisalmer, royal palaces of Udaipur to the tigers of Ranthambore — Rajasthan is India's crown jewel of heritage and adventure.</p>
                <div className="flex flex-wrap gap-6 mt-6">
                  <div className="flex items-center gap-2 text-sm"><MapPin size={16} className="text-gold-400" /> <strong>{destinations.length || '30+'}</strong> Destinations</div>
                  <div className="flex items-center gap-2 text-sm"><Award size={16} className="text-gold-400" /> Royal Heritage</div>
                  <div className="flex items-center gap-2 text-sm"><Heart size={16} className="text-gold-400" /> Desert Safari</div>
      ...
      <WhyTravelSection title="Why Travel Rajasthan?" subtitle="Discover Rajasthan" items={WHY_RAJASTHAN} />
        ...
        <section className="section-padding bg-gray-50">
          <div className="container-wide">
            <div className="text-center mb-10">
              <span className="text-gold-600 font-semibold text-sm uppercase tracking-wider">Explore Rajasthan</span>
              <h2 className="text-3xl md:text-4xl font-display font-bold text-navy-900 mt-2">Popular Destinations in Rajasthan</h2>
              <p className="text-navy-500 mt-3 max-w-xl mx-auto">From the Pink City to the Golden City — discover Rajasthan's iconic destinations</p>
            </div>
                ...
                <Link key={d.id || d.slug} to={`/destinations/${d.slug || d.id}`}
                  ...
                  <div className="aspect-[4/3] overflow-hidden">
                    {(d.image || d.coverImage) ? (
                      ...
                      <div className="w-full h-full bg-gradient-to-br from-gold-400 to-navy-600 flex items-center justify-center text-white text-3xl">🏰</div>
                    )}
                  </div>
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent" />
                  <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
                    <h3 className="font-bold text-lg">{d.name}</h3>
                    <p className="text-sm text-gray-200">{d.tagline || d.state || 'Rajasthan'}</p>
      ...
      {/* ═══ ALL RAJASTHAN PACKAGES ═══ */}
      <StatePackagesSection stateName="Rajasthan" />
      ...
      <section className="section-padding bg-gray-50">
        <div className="container-wide">
          <div className="text-center mb-10">
            ...
            <h2 className="text-3xl md:text-4xl font-display font-bold text-navy-900 mt-2">Rajasthan in Pictures</h2>
            <p className="text-navy-500 mt-3">A visual journey through Rajasthan's most breathtaking destinations</p>
          </div>
      ...
      <section className="bg-gradient-to-r from-navy-900 to-gold-900 py-8">
        <div className="container-wide">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center text-white">
            <div>
              <div className="text-3xl font-bold">{destinations.length || '30+'}+</div>
              <div className="text-sm text-gray-300 mt-1">Destinations</div>
            </div>
            <div>
              <div className="text-3xl font-bold">{packages.length || '50+'}+</div>
        ...
        <div className="container-wide">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-display font-bold text-navy-900 mt-2 mb-6">Plan Your Rajasthan Trip</h2>
              <p className="text-navy-500 mb-8">Ready to explore Rajasthan? Contact our travel experts for a customized itinerary, best deals, and hassle-free booking experience.</p>
              <div className="space-y-5">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-navy-900">Jaipur, Rajasthan, India</p>
                  </div>
                </div>
              </div>
            </div>
            <div className="bg-gray-50 rounded-2xl shadow-lg p-8">
              ...
              <form onSubmit={e => { e.preventDefault(); alert('Thank you! Our team will contact you shortly.') }} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <input type="text" placeholder="Preferred Destination" defaultValue="Rajasthan" className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-sky-500 focus:outline-none" />
                </div>
                <div className="grid grid-cols-2 gap-4">
